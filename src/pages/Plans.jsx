...
import { getDB, addInvestmentRequest, getSettings } from '../lib/storage';
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Convert to low-quality JPEG to save space in localStorage
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '30px' }}>
        {plans.map((plan) => (
          <motion.div 
            key={plan.id}
            whileHover={{ y: -10 }}
            className={`card glass ${user.plan?.id === plan.id ? 'active-plan' : ''}`}
            style={{ 
              padding: '40px',
              border: user.plan?.id === plan.id ? '2px solid var(--primary)' : '1px solid var(--glass-border)',
              display: 'flex',
          ...
          >
            ...
            <button 
              ...
              className={user.plan?.id === plan.id ? 'glass' : 'gradient-btn'}
            ...
            >
              {user.plan?.id === plan.id ? 'Already Subscribed' : (
